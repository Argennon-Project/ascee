...
static
bool verifyWithMultiwayNonce(int nonceCount, long_id issuer, long_id spender, message_c& msg, int16 sigIndex,
                             int32& balanceIndex, bool invalidate) {
    auto& heap = Executor::getSession()->heapModifier;

    balanceIndex = decision_nonce_size_g + public_key_size_k;
    if (msg.size() == 0) return true;

    if (!invalidate) {
        return Executor::getSession()->sigManager.verify(string_view(msg), issuer, sigIndex);
    }

    auto nonceIndex = 0;
    if (nonceCount > 1) {
        nonceIndex = balanceIndex + arg_balance_size_g + int32(string_view(msg).length() % nonceCount) * nonce16_size_g;
    }
    auto nonce = heap.load<uint16>(nonceIndex);
    if (nonce >= nonce16_max) return false;

    appendNonceToMsg(msg, spender, nonce);
    bool valid = Executor::getSession()->sigManager.verify(string_view(msg), issuer, sigIndex);
...
static
bool verifyByApp(long_id issuer, long_id spender, message_c& msg, int16 sigIndex, bool invalidateMsg) {
    if (msg.size() == 0) return true;
    if (!invalidateMsg) {
        return Executor::getSession()->sigManager.verify(string_view(msg), issuer, sigIndex);
    }
    appendSpenderToMsg(msg, spender);
    return Executor::getSession()->sigManager.verifyAndInvalidate(string_view(msg), issuer, sigIndex);
}

bool verifyByAccount(long_id issuerAccount, message_c& msg, int16 sigIndex, int32& balanceIndex, bool invalidateMsg) {
    Context context;
    long_id spender = context.caller;
    auto& heap = Executor::getSession()->heapModifier;

    heap.loadChunk(issuerAccount, nonce_chunk_local_id_g);
    ...
    if (decisionNonce == 0) {
        ...
        return verifyByApp(issuerAccount, spender, msg, sigIndex, invalidateMsg);
    }
    ...
    if (decisionNonce == 1) {
        return verifyWithMultiwayNonce(5, issuerAccount, spender, msg, sigIndex, balanceIndex, invalidateMsg);
    }

    // decisionNonce == 2 means that the owner account has an 11 way nonce value.
    if (decisionNonce == 2) {
        return verifyWithMultiwayNonce(11, issuerAccount, spender, msg, sigIndex, balanceIndex, invalidateMsg);
    }
    ...
    if (decisionNonce >= min_nonce_g) {
        return verifyWithMultiwayNonce(1, issuerAccount, spender, msg, sigIndex, balanceIndex, invalidateMsg);
    }
...
int16 argc::virtual_sign(long_id issuer_account, message_c& msg) {
    ...
    long_id owner = heap.loadIdentifier(app_trie_g, decision_nonce_size_g + arg_balance_size_g, 0);
    if (signer != owner) {
...
bool argc::verify_by_acc_once(long_id issuer_account, message_c& msg, int16 sigIndex, int32& balance_offset) {
    Executor::guardArea();
    auto ret = verifyByAccount(issuer_account, msg, sigIndex, balance_offset, true);
    Executor::unGuard();
    return ret;
}

bool argc::verify_by_acc(long_id issuer_account, message_c& msg, int16 sigIndex) {
    int32 dummy;
    return verifyByAccount(issuer_account, msg, sigIndex, dummy, false);
}
