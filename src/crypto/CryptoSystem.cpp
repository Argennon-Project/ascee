...
int CryptoSystem::getDigest(const StringView& msg, unsigned char* digest) {
    unsigned int len = 0;
    EVP_Digest((void*) msg.data(), msg.length(), digest, &len, EVP_sha3_256(), nullptr);
    return int(len);
}

Signature CryptoSystem::sign(const StringView& msg, SecretKey& sk) {
...
bool CryptoSystem::verify(const StringView& msg, Signature& sig, PublicKey& pk) {
