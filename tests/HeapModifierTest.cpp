...
protected:
    heap::Chunk tempChunk1_1;
    heap::Chunk tempChunk1_2;
    heap::Chunk tempChunk1_3;
    heap::Chunk tempChunk2_1;
    heap::Chunk tempChunk2_2;
...
public:
    HeapModifierTest() {
        tempChunk1_1.reserveSpace(256);
        tempChunk1_2.reserveSpace(256);
        tempChunk1_3.reserveSpace(256);
        tempChunk2_1.reserveSpace(256);
        tempChunk2_1.reserveSpace(256);

        tempChunk1_1.setSize(256);
        tempChunk1_2.setSize(256);
        tempChunk1_3.setSize(256);
        ...
        chunksForApp1.emplace_back(&tempChunk1_1, 0, false,
                                   vector<int32>{100, 108, 253},
                                   vector<BlockAccessInfo>{
                                           {8,  false, 0},
                                           {16, true,  0},
                                           {4,  false, 0},
                                   });
        chunksForApp1.emplace_back(&tempChunk1_2, 0, false,
                                   vector<int32>{100, 120},
                                   vector<BlockAccessInfo>{
                                           {8, true, 0},
                                           {8, true, 0},
                                   });
        chunksForApp1.emplace_back(&tempChunk1_3, 0, false,
                                   vector<int32>{100},
                                   vector<BlockAccessInfo>{
                                           {8, true, 0},
                                   });
...
TEST_F(HeapModifierTest, VersionZero) {
    modifier->loadContext(1);
    modifier->loadChunk(short_id(10));
    auto got = modifier->load<int64>(100);
    EXPECT_EQ(got, 0) << "got: 0x" << std::hex << got;

    EXPECT_THROW(modifier->load<int64>(0), std::out_of_range);

    EXPECT_THROW(modifier->load<int32>(253), std::out_of_range);
...
TEST_F(HeapModifierTest, RestoringMultiVersions) {
    modifier->loadContext(1);
    modifier->loadChunk(short_id(11));
